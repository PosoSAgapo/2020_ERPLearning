...
class NeuralTensorNetwork_yago(nn.Module):
    def __init__(self, embeddings, k):
        super(NeuralTensorNetwork_yago, self).__init__()
        self.embeddings = embeddings
        self.vocab_size, self.emb_dim = embeddings.weight.size()
        self.k = k
        self.subj_verb_comp = TensorComposition(k, self.emb_dim, self.emb_dim)
    ...
    def forward(self, subj_id, subj_w, verb_id, verb_w):
        '''
        subj_id: (batch, n)
        subj_w:  (batch, n)
        verb_id: (batch, n)
...
class MarginLoss(nn.Module):
    ...
    def forward(self, pos_score, neg_score):
        ...
        neg_score: (batch)
        '''
        loss = self.relu(neg_score - pos_score + self.margin)
